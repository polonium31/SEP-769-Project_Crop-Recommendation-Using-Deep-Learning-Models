...
file_path = "/content/drive/MyDrive/Group_4_S25/Crop_recommendation.csv"
...
# Find the best model index by Accuracy or Macro F1-Score (choose metric)
best_model_idx = results['Accuracy'].idxmax()  # Or 'Macro F1-Score'
...
best_model = dnn_model_50
best_history = history_dnn_50
...
def predict_crop(user_input, model, crop_names=None):
    """
    Predict crop class given input features using the loaded model.
    ...
    # Prepare input for model: reshape to (1, 7, 1)
    input_array = np.array(user_input).reshape(1, 7, 1)
    ...
    return predicted_class, predicted_crop_name, probabilities[0]
...
predicted_class, predicted_crop_name, probs = predict_crop(user_input, cnn_model_50, crop_names)
...
# Save model
cnn_model_50.save('best_model.h5')
